...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const select = metaOnly ? 'id,updated_at' : 'id,updated_at,data';
    ...
    if (req.method === 'POST') {
      ...
      const checkR = await fetch(restUrl() + '?id=eq.' + ROW_ID + '&select=id,updated_at,data', {
        headers: supabaseHeaders()
      });
      ...
      const writeR = await fetch(restUrl() + '?on_conflict=id', {
        ...
        body: JSON.stringify({ id: ROW_ID, updated_at: nowIso, data: snapshot })
      });
      if (!writeR.ok) {
